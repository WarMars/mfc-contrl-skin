...
public:
	...
	/*void GetAttrib( int *nValue)
	{
		MCD_STR  s = __NAME__(nValue);
		*nValue = atoi( x_GetAttrib(m_iPos,s.c_str()).c_str());
	}
	void GetAttrib( float *fValue)
	{
		MCD_STR  s = __NAME__(nValue);
		::OutputDebugString( s.c_str());
		::OutputDebugString( "------------------");
		*fValue = atof( x_GetAttrib(m_iPos,s.c_str()).c_str());
	}*/
	...
	bool AddAttrib(MCD_CSTR szAttrib, double fValue)
	{
		MCD_CHAR pcsz[256]={0}; 
		swprintf_s( pcsz, 255, _T("%0.3f"), fValue);
		return x_SetAttrib(m_iPos,szAttrib,pcsz);
	};
	bool AddAttrib(MCD_CSTR szAttrib, unsigned long dValue)
	{ 
		MCD_CHAR pcsz[256]={0}; 
		swprintf_s( pcsz, 255, _T("%u"), dValue);
		return x_SetAttrib(m_iPos,szAttrib,pcsz);
	};
