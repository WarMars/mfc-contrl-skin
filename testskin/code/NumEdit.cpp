...
//	   Note refer WINUSER.H for virtual key definition eg VK_SPACE = 0x20
	bool bIsNumber = (nChar >= '0'   &&  nChar <= '9');
	if((nChar <  VK_SPACE ) ||bIsNumber)
	{
		 bValidChar = true;
	}
...
void   CNumEdit::Set_EditCtrl_Text(std::wstring strText)
{
...
double  CNumEdit::Text_To_Number(const wchar_t* strText)
{
	return _wtof(strText);
}
//--------------------------------------------------------------------------------------/

std::wstring CNumEdit::Number_To_Text(double fNumber)
{
// 1 - Prepare the format string 
//	   eg format string %.5f formats a double with the precision set 
//	   to five (5)
	wchar_t strFormat[128] = {0};
	swprintf_s(strFormat, 127, _T("%.2f"),fNumber);
	m_strText = strFormat;
...
std::wstring CNumEdit::Get_Number_Text()
{
	return m_strText;
}
//--------------------------------------------------------------------------------------/
